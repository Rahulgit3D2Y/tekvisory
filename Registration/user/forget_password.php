...
<div class="col-md-6 panel" style="background-image:url(../admin/image/bg1.jpg); min-height:100px;">
     <br> <br>
...
//echo $passgen;
    ...
    $date = $_POST['dob'];
    $number = $_POST['number'];
$select = "SELECT `student_dob`,`student_number`,`fee_status` FROM `registration` WHERE `login` = '$login'and `fee_status` = \"Active\"";
...
$olddob = $data['student_dob'];
$num= $data['student_number'];
    if ($olddob==$date && $num==$number ) 
    {
        
            $update = "UPDATE `registration` SET `password`='$hash_pass',`pass`='$passgen',`forget_password_time`='$currentdate',`wrong_pass_count`='0',`wrong_pass_time`='' WHERE `login` ='$login'";
