...
$select = "SELECT `password`,`hash_password` FROM `registration` WHERE `login` = '$login'";
   ...
   if (($oldp==$old) || ($roldp==$old)) 
    {
        if ($new == $conpass) 
        {
            $update = "UPDATE `registration` SET `password` = '$new',`pass` = '$new1',`oldpass` = '$old1',`change_pass_time` = '$date' WHERE `login` = '$login' and `password` = '$old' or `login` = '$login' and `hash_password` = '$old' ";
...
<div class="col-md-6 panel" style="background-image:url(../admin/image/bg1.jpg); min-height:200px;">
     <br>
