        ...
        if(isset($CertificateCheck))
{
   ...
   $updatedatatcoursequery=mysqli_query($con,"SELECT * FROM `registration` WHERE `student_id`='$StudentId' AND `student_email`='$studentEmail' AND `student_dob`='$studentdob' AND `fee_status`='Active';");
...
$receipt_number=$result['receipt_number'];
    ...
    $image=imagecreatefromjpeg("FinalTechWizardCertificate.jpg");
    $color=imagecolorallocate($image,255, 255, 255);
    
    imagettftext($image,50,1,730,605,$color,"FontsFree-Net-Poppins-Medium.TTF",$student_name);
    //$date=$college_name;
    imagettftext($image,35,1,530,745,$color,"FontsFree-Net-Poppins-Medium.TTF",$college_name);
    imagettftext($image,20,1,300,1410,$color,"FontsFree-Net-Poppins-Medium.TTF",$receipt_number);
