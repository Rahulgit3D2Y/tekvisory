...
<head>
    ...
    <meta name="description" content="GEU-Department Of Computer Applications">
...
//$message= "YOUR ADMISSION IS SUCCESSFULLY COMPLETED IN $student_result_coursename\n YOUR STUDENT ID IS $id \n AND PASSWORD WILL BE SAME AS USERID;
 // $rs=mysqli_query($con,"select * from course where course_id2='$inputFirstName' or course_name='$inputLastName'");
    $rs=mysqli_query($con,"SELECT * FROM `user` WHERE `first_name`='$inputFirstName' OR `number`='$inputStudentNumber' OR `email` = '$inputStudentEmail' AND  `dob` = '$inputDateofbirth' AND `usercourse_id`= '$inputStudentCourse' ");
 ...
 if (mysqli_num_rows($idquerygen)>0)
{
 ...
 mysqli_query($con,"INSERT INTO `user`(`first_name`, `middle_name`, `last_name`, `login`, `user_id`, `password`, `photo`,`dob`, `number`, `email`, `type`, `status`, `session`, `createdtime`,`usercourse_id`,`usersection`,`usersemester`) VALUES ('$inputFirstName','$inputMiddleName','$inputLastName','$id','$id','$password_gen','$inputuserphotoupload','$inputDateofbirth','$inputStudentNumber','$inputStudentEmail','user','$status','$studentsessionyearrecord','$date','$inputStudentCourse','$inputStudentSection','$inputStudentSemester')") or die(mysqli_error());
...
mysqli_query($con,"INSERT INTO `user`(`first_name`, `middle_name`, `last_name`, `login`, `user_id`, `password`, `photo`, `dob`, `number`, `email`, `type`, `status`, `session`, `createdtime`,`usercourse_id`,`usersection`,`usersemester`) VALUES ('$inputFirstName','$inputMiddleName','$inputLastName','$id1','$id1','$password_gen1','$inputuserphotoupload','$inputDateofbirth','$inputStudentNumber','$inputStudentEmail','user','$status','$studentsessionyearrecord','$date','$inputStudentCourse','$inputStudentSection','$inputStudentSemester')") or die(mysqli_error());
                    ...
                    <div class="col-lg-12">
                        <div class="p-5">
                            ...
                            <form  action="User_SelfRegistration.php" method="POST" onSubmit="return check();" enctype="multipart/form-data">
                               ...
                               <!--<div class="form-group row">
                                    <div class="col-sm-4 mb-3 mb-sm-0">
                                        <select class="form-control" name="inputStudentCourse" id="inputStudentCourse" required="inputStudentCourse">
          <option selected="selected" value=""disabled selected>-- Select an option --</option>
           <?php
$rscourse=mysqli_query($con,"SELECT course_detail.`course_id`,course_detail.`course_id2`,course_detail.`course_name` FROM `course_detail`   WHERE `status` = \"Active\" ");
...
?></select>
                                    ...
                                    <div class="col-sm-4 mb-3 mb-sm-0">
                                       <select class="form-control" name="inputStudentSection" id="inputStudentSection" required="inputStudentSection" >
          <option selected="selected" value=""disabled selected>-- Select an option --</option>
            <?php for($i='A'; $i<'Z'; $i++) { ?>
  <option  value="<?php echo $i; ?>"><?php echo $i; ?></option>
        ...
        <?php } ?>
                                    ...
                                    <div class="col-sm-4 mb-3 mb-sm-0">
                                       <select class="form-control" name="inputStudentSemester" id="inputStudentSemester" required="inputStudentSemester" >
          <option selected="selected" value=""disabled selected>-- Select an option --</option>
              <?php for($i=1; $i<=10; $i++) { ?>
  <option  value="<?php echo $i; ?>"><?php echo $i; ?></option>
